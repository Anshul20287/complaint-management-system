...
const ZoneMap = () => {
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      try {
        setLoading(true);
        // Fetch all complaints, we'll filter by domain
  ...
  // Filter complaints by selected domain/category
  const zoneComplaints = selectedDomain
    ? complaints.filter(c => c.category === selectedDomain)
    : complaints;
  ...
  const stats = {
    ...
    open: zoneComplaints.filter(c => c.status === 'OPEN').length,
    inProgress: zoneComplaints.filter(c => c.status === 'IN_PROGRESS').length,
    resolved: zoneComplaints.filter(c => c.status === 'RESOLVED').length,
  };
    ...
    return (
      <div className="rounded-2xl border border-cyan-400/10 bg-[#09111f]/70 p-6 backdrop-blur-sm">
        <h2 className="mb-4 text-base font-semibold text-[#e6f1ff]">
          My Zone · {selectedDomain || 'All'}
        </h2>
        <div className="flex items-center justify-center h-40">
          <div className="text-[#7c8aa5]">Loading zone data...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-cyan-400/10 bg-[#09111f]/70 p-6 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-[#e6f1ff]">
          My Zone · {selectedDomain || 'Unassigned'}
        </h2>
        <span className="text-xs px-2 py-1 rounded-full bg-cyan-400/10 border border-cyan-400/30 text-cyan-300 font-medium">
          {stats.total} issues
        </span>
      </div>
          ...
          {zoneComplaints.length > 0 ? (
            ...
            <div className="flex h-full items-center justify-center">
              <div className="text-center">
                <p className="text-[#7c8aa5]">No issues in this zone</p>
              </div>
            </div>
          )}
        </div>
